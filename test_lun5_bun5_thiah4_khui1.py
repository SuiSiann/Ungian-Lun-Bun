...
from lun5_bun5_thiah4_khui1 import han_hokbu_lineregex, han_lineregex,\
    lo_lineregex, bracketregex, subtitle_regex
...
class 拆開論文單元試驗(TestCase):
    ...
    def test_ah(self):
        ...
        self.assertEqual(len(han_arr), 9, han_arr)
    ...
    def test_al(self):
        ...
        self.assertEqual(len(lo_arr), 9, lo_arr)
    ...
    def test_bh(self):
        ...
        han_arr = han_lineregex.findall(han)
        self.assertEqual(len(han_arr), 11, han_arr)

    @skip('暫時靠人工處理')
    def test_數字有半形逗號和小數點(self):
    ...
    def test_ch(self):
        han = "實驗ê結果並無原來按算ê hiah-nī好，下面分析問題出tī tó位："
        han_arr = han_lineregex.findall(han)
        self.assertEqual(len(han_arr), 2, han_arr)

    def test_cl(self):
        lo = "Si̍t-giām ê kiat-kó pēng bô gôan-lâi àn-sǹg ê hiah-nī hó, ē-bīn hun-sek būn-tôe chhut tī tó-ūi :"
        lo_arr = lo_lineregex.findall(lo)
        self.assertEqual(len(lo_arr), 2, lo_arr)

    def test_副標題(self):
        tsua = "5.1 MI kah CR 共同詞組數量 [MI kah CR kiōng-tông sû-cho͘ sò͘-liōng]"
        kiat_ko = subtitle_regex.match(tsua)
        self.assertTrue(kiat_ko)
